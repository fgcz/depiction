...
There is no helper here. Writing OME-NGFF happens in one place only, the pipeline step
`depiction_targeted_preproc.workflow.vis.images_ome_ngff`, which hands a `MultiChannelImage`
to `bioio.writers.OmeZarrWriter` directly. If a second caller ever appears, that is the point
to extract a helper alongside `OmeTiff`.
