...
See [../ROADMAP.md](../ROADMAP.md) for what was done with it.

## Source code (from the `separate-depiction-io` worktree, now tag `archive/separate-depiction-io`)
...
They resolve the `MultiChannelImage` circular-dependency knot and were reused in roadmap
Phase B, which has since landed.
...
| `test_ome_tiff.py.rescued` | `tests/unit/image/test_ome_tiff.py` — differs from what was then `tests/unit/persistence/image/test_ome_tiff.py`, now `tests/unit/image/test_ome_tiff.py`, by one import line only |
...
| `2025-03-multi-channel-image-plan.md` | `.local/refactor_depiction_image.md` | Never started. Splits `MultiChannelImage` into masked/unmasked and renames `AlphaChannel`→`MaskChannel`. **Deliberately out of scope** for the current roadmap. Only other trace is tag `archive/better-bg-handling`. |
