...
from depiction_io import ImzmlReadFile, ImzmlModeEnum
...
def test_get_reader(mocker: MockerFixture, mock_path: Path, mock_read_file: ImzmlReadFile) -> None:
    mock_etree = mocker.patch("depiction_io.imzml.imzml_reader.ElementTree").return_value
    mock_parse_spectra = mocker.MagicMock(name="mock_parse_spectra", spec=ParseSpectra)
    construct_parse_spectra = mocker.patch(
        "depiction_io.imzml.imzml_reader.ParseSpectra", return_value=mock_parse_spectra
    )
...
def test_metadata_checksums(mocker: MockerFixture, mock_read_file: ImzmlReadFile) -> None:
    mock_etree = mocker.patch("depiction_io.imzml.imzml_read_file.ElementTree").return_value
    mock_parse_metadata = mocker.patch("depiction_io.imzml.imzml_read_file.ParseMetadata")
...
def test_ibd_checksums(mocker: MockerFixture, mock_read_file: ImzmlReadFile) -> None:
    mock_file_checksums = mocker.patch("depiction_io.imzml.imzml_read_file.FileChecksums")
