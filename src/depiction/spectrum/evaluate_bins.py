...
class EvaluateBins:
    ...
    def evaluate(self, mz_arr: NDArray[np.float64], int_arr: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        """
        # `np.issubdtype`, not `isinstance`: this test used to read
        # `isinstance(mz_arr.dtype, (float, np.float64))`, and `mz_arr.dtype` is a `numpy.dtype`
        # object, never an instance of either -- so it was always False and every binning silently
        # ran in float32, including for the float64 input this branch exists to preserve.
        is_f64 = np.issubdtype(mz_arr.dtype, np.float64) or np.issubdtype(int_arr.dtype, np.float64)
