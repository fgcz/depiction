...
class TestSpatialSmoothingSparseAware(unittest.TestCase):
    ...
    def test_smooth_sparse(self) -> None:
    ...
    def test_smooth_sparse_preserves_values(self, fill_value) -> None:
    ...
    @given(strategies.floats(min_value=0, max_value=1e12, allow_subnormal=False))
    @settings(deadline=timedelta(seconds=1))
    def test_smooth_sparse_non_square(self, fill_value) -> None:
        mock_sparse = np.full(20, fill_value=fill_value)
        mock_coordinates = np.array([[i // 3, i % 3] for i in range(20)])
        smoothed = self.mock_target.smooth_sparse(sparse_values=mock_sparse, coordinates=mock_coordinates)
        np.testing.assert_allclose(mock_sparse, smoothed, rtol=1e-8)

    def test_smooth_sparse_casts_when_integer(self) -> None:
    ...
    def test_smooth_dense(self) -> None:
    ...
    def test_smooth_dense_preserves_values(self) -> None:
    ...
    def test_smooth_sparse_multi_channel(self) -> None:
    ...
    def test_smooth_dense_multi_channel(self) -> None:
        image_2d = np.stack(
