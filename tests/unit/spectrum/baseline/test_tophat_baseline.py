...
class TestTophatBaseline(unittest.TestCase):
    def setUp(self) -> None:
    ...
    @cached_property
    def mock_baseline(self) -> TophatBaseline:
        # noinspection PyTypeChecker
        return TophatBaseline(window_size=self.mock_window_size, window_unit=self.mock_window_unit)
    ...
    def test_evaluate_baseline(self) -> None:
        ...
        baseline = self.mock_baseline.evaluate_baseline(mock_mz_arr, x)
        np.testing.assert_array_equal(np.full_like(x, 10.0), baseline)

    @patch.object(TophatBaseline, "evaluate_baseline")
    ...
    def test_get_element_size_when_index(self) -> None:
        ...
        self.assertEqual(5, self.mock_baseline.get_element_size(mz_arr=mock_mz_arr))
    ...
    def test_get_element_size_when_ppm(self) -> None:
        ...
        self.assertEqual(10, self.mock_baseline.get_element_size(mz_arr=mock_mz_arr))

    def test_get_element_size_when_invalid(self) -> None:
        self.mock_window_unit = "mz"
        mock_mz_arr = MagicMock(name="mock_mz_arr", spec=[])
        with self.assertRaises(ValueError):
            self.mock_baseline.get_element_size(mz_arr=mock_mz_arr)
