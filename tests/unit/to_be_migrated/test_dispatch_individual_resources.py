...
def test_dispatch_workunit_when_resources(mocker, mock_dispatch):
    ...
    write_chunks.assert_called_once_with(
        out_dir=Path("/dev/null"), chunks=mock_dispatch._dispatch_jobs_resource_flow.return_value
    )
...
def test_dispatch_workunit_when_dataset(mocker, mock_dispatch):
    ...
    dispatch_jobs.assert_called_once_with(mock_definition, mock_definition.execution.raw_parameters)
    write_chunks.assert_called_once_with(
        out_dir=Path("/dev/null"), chunks=mock_dispatch._dispatch_jobs_dataset_flow.return_value
    )
    write_workunit_definition.assert_called_once_with(out_dir=Path("/dev/null"), definition=mock_definition)
